...
use super::{
    ...
    layout::{
        DistrictsWording, bold_value_section, candidates_section, column_table, districts_section,
        election_section, signature_line, start_h_document, translator, warning,
    },
};
...
impl Pdf for H9 {
    fn document(&self) -> Textris {
        ...
        self.representative_section(&mut doc);
        self.notification_section(&mut doc);
        self.candidate_signature(&mut doc);
...
impl H9 {
    /// The numbered "Gemachtigde" section; only present when the candidate has
    /// a representative.
    fn representative_section(&self, doc: &mut Textris) {
        let Some(representative) = &self.detailed_candidate.representative else {
            return;
        };
        let trans = translator(self.common.locale);
    ...
    /// The numbered notification section for candidates without a
    /// representative: the notification address, or for the non-resident
    /// electoral college the digital-notification consent.
    fn notification_section(&self, doc: &mut Textris) {
        let candidate = &self.detailed_candidate;
        if candidate.representative.is_some() {
            return;
        }
        let trans = translator(self.common.locale);

        if self.common.election_type == ElectionType::Kcni {
            // a postal address does not apply to the election of the electoral
    ...
    /// The numbered "Ondertekening" section: the candidate's details and the
    /// signature line.
    fn candidate_signature(&self, doc: &mut Textris) {
        let trans = translator(self.common.locale);
        let candidate = &self.detailed_candidate;
        doc.h3_numbered(trans(
