    ...
    fn from(
        ...
        let added_candidates = match added_position {
            Some(pos) => full_list
                .candidates
                .iter()
                .filter(|candidate| candidate.candidate.position >= pos)
                .map(|candidate| (candidate.candidate.person.id, candidate.candidate.position))
...
mod tests {
    ...
    async fn add_person_to_candidate_list_adds_and_renders() -> Result<(), AppError> {
        ...
        assert_eq!(full_list.candidates[0].candidate.person.id, person.id);
    ...
    async fn add_person_to_candidate_list_adds_when_person_not_on_list() -> Result<(), AppError> {
        ...
        assert_eq!(full_list.candidates[0].candidate.person.id, existing_person.id);
        assert_eq!(full_list.candidates[1].candidate.person.id, new_person.id);
    ...
    async fn add_person_to_candidate_list_removes_candidate() -> Result<(), AppError> {
        ...
        let full_list = FullCandidateList::get(&store, list_id).expect("candidate list");
        assert_eq!(full_list.candidates.len(), 1);
        assert_eq!(full_list.candidates[0].candidate.person.id, keep_person.id);
