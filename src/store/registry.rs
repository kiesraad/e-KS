...
type StoreKey = (StreamId, ElectionConfig, Scope);
    ...
    D::Event: Serialize + DeserializeOwned,
{
    ...
    pub async fn get_store(
        ...
        self.inner
            .read()
            .get(&(stream_id, election, D::scope()))
            .cloned()
    ...
    {
        let key = (stream_id, election, D::scope());
    ...
    pub async fn elections_for_stream(
        ...
        let mut found: HashSet<ElectionConfig> = {
            ...
            cached
                .iter()
                .filter_map(|((id, election, scope), store)| {
                    (*id == stream_id
                        && store.data.read().last_event_id() > 0
                        && scope == &D::scope())
                        .then_some(*election)
                })
                .collect()
