...
use crate::{
    AppState, ElectionConfig, Locale, PgStore, Session, StreamId,
    persons::PersonId,
    test_utils::{response_body_string, sample_person, sample_political_group},
};

async fn setup() -> (Router, PgStore, String) {
    ...
    (app, PgStore::own(store), token)
}

fn get_request(uri: &str, token: &str, store: PgStore) -> Request<Body> {
