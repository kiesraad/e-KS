...
use crate::{
    AppError, AppState, Config, ElectionConfig, Locale, PgEvent, PgStore, Session, StreamId,
...
async fn setup_app() -> Result<(Router, PgStore, Session), AppError> {
...
fn request(uri: String, session: Session, store: PgStore) -> Request<Body> {
