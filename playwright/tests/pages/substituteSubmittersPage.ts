...
export class SubstituteSubmittersPage {
  ...
  async open() {
    await this.page.goto("/political-group/substitute-submitters");
  }
  ...
  for (const href of hrefs) {
        if (href) {
          ...
          await this.page.getByRole("button", {name: "Vervangende lijstinleveraar verwijderen", exact: true }).click();
  ...
  async addSubstituteSubmitter(listSubmitters: ListSubmitter[]) {
    ...
    await this.page.getByRole("link", {name: "Vervangende lijstinleveraar toevoegen"}).click();
    await this.page.getByRole("textbox", { name: "Voorletters *" }).fill(listSubmitter.initials);
    await this.page.getByRole("textbox", { name: "Voorvoegsel" }).fill(listSubmitter.lastNamePrefix ?? "");
    await this.page.getByRole("textbox", { name: "Achternaam *" }).fill(listSubmitter.lastName);
    await this.page.getByRole("button", {name: "Opslaan"}).click();
  }
  }

    async editListSubmitter() {
    await this.page.getByRole("cell", {name: "Vervangende lijstinleveraar toevoegen"}).click();
