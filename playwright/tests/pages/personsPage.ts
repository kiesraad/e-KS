...
export class PersonsPage {
    ...
    async addPersons(candidates: Candidate[]) {
        for(var candidate of candidates) {
            ...
            await this.page.locator('input[name="last_name_prefix"]').fill(candidate.lastNamePrefix ?? '');
            ...
            await expect(this.page.getByRole('textbox', { name: 'Street name' })).toHaveValue(candidate.streetName ?? '');
            await expect(this.page.getByRole('combobox', { name: 'Locality' })).toHaveValue(candidate.locality ?? '');
