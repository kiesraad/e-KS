...
test.describe("import and export candidates with csv file", () => {
  ...
  async function navigateToCsvPage(page: Page) {
    ...
    await new CandidateListsOverviewPage(page).linkCandidateList.first().click();
  ...
  async function uploadCsvFile(page: Page, csvImportExport: CsvImportExportPage, filename: string) {
  ...
  test("import successfull", async ({ login: page }) => {
    const csvImportExport = await navigateToCsvPage(page);
    await uploadCsvFile(page, csvImportExport, "candidate-list-export-nh-1.csv");
    ...
    await expect(await manageCandidateListPage.getCandidateLocator("Groot, de")).toBeVisible();
  });
